...
# ---------------------------------------------------------
# Reuses Step 1 (run_procurement_scoring), Step 2 (run_risk_intelligence),
# and Step 3 (run_savings_intelligence) exactly as-is - no formulas are
# duplicated here. This step only orchestrates and compares their outputs.
...
WEIGHT_LABELS = {
    ...
    "warranty": "Warranty priority",
}

REQUIREMENT_LABELS = {
    "budget": "Budget",
    "required_delivery_days": "Required delivery days",
    "minimum_quality": "Minimum quality",
    "minimum_reliability": "Minimum reliability",
...
def diff_weights(original_weights, new_weights):
    """Returns a list of (criterion, old_value, new_value) for every changed weight."""
    changes = []
...
def diff_requirements(original_requirements, new_requirements):
    """Returns a list of (field, old_value, new_value) for every changed requirement."""
    changes = []
    for key in original_requirements:
        old_val = original_requirements[key]
        new_val = new_requirements.get(key, old_val)
        if old_val != new_val:
            changes.append((key, old_val, new_val))
    return changes


def compute_whatif_state(vendors, weights, requirements, risk_weights):
    """
    Builds one full snapshot (scoring + risk + savings) for a given set of
    weights/requirements, entirely by calling the existing Step 1-3 engines.
    """
    scoring_result = run_procurement_scoring(vendors, weights)

    recommended_name = scoring_result["recommended_vendor"]
    recommended_score = scoring_result["recommended_score"]

    risk_assessments = run_risk_intelligence(vendors, requirements, risk_weights)
    recommended_risk = next(a for a in risk_assessments if a["name"] == recommended_name)

    savings_list = run_savings_intelligence(vendors, requirements["budget"])
    recommended_savings = next(s for s in savings_list if s["name"] == recommended_name)
...
def generate_recommendation_change_reason(before_state, after_state, weight_changes):
    """
    When the recommended vendor changes, explains why using the actual
    normalized scores and weighted contributions from the AFTER state.
    """
    old_name = before_state["recommended_name"]
    new_name = after_state["recommended_name"]

    after_vendors = {v["name"]: v for v in after_state["scoring_result"]["vendors"]}
    new_winner = after_vendors[new_name]
    old_leader_now = after_vendors[old_name]

    if weight_changes:
        # Criterion with the largest weight increase is treated as the primary driver
        biggest = max(weight_changes, key=lambda c: c[2] - c[1])
        crit, old_w, new_w = biggest
        crit_label = CRITERION_LABELS[crit]

        new_norm = new_winner["normalized"][crit]
        old_norm = old_leader_now["normalized"][crit]
        new_contrib = new_winner["contributions"][crit]
        old_contrib = old_leader_now["contributions"][crit]

        return (
            f"{new_name} overtook {old_name} because {crit_label} weight rose from "
            f"{old_w}% to {new_w}%. {new_name}'s {crit_label} score "
            f"({new_norm:.2f}/100) outperforms {old_name}'s ({old_norm:.2f}/100), "
            f"contributing {new_contrib:.2f} points versus {old_contrib:.2f} points "
            f"toward the overall score."
        )
    else:
        return (
            f"{new_name} now scores {after_state['recommended_score']:.2f} versus "
            f"{old_name}'s {old_leader_now['overall_score']:.2f} under the updated conditions."
        )


def generate_whatif_explanation(before_state, after_state, weight_changes, requirement_changes):
    """
    Produces the full WHY explanation covering recommendation, risk, and
    savings movement, built entirely from calculated before/after numbers.
    """
    parts = []

    if before_state["recommended_name"] != after_state["recommended_name"]:
        parts.append(generate_recommendation_change_reason(before_state, after_state, weight_changes))
    else:
        name = after_state["recommended_name"]
        parts.append(
            f"Recommendation remained unchanged because {name} still leads with the "
            f"highest overall score ({before_state['recommended_score']:.2f} → "
            f"{after_state['recommended_score']:.2f})."
        )

    # Risk movement for whichever vendor ends up recommended after the change
    after_name = after_state["recommended_name"]
    before_risk_for_after_vendor = next(
        a for a in before_state["risk_assessments"] if a["name"] == after_name
    )
    after_risk_for_after_vendor = after_state["recommended_risk"]

    if abs(before_risk_for_after_vendor["overall_risk"] - after_risk_for_after_vendor["overall_risk"]) > 0.01:
        parts.append(
            f"{after_name}'s risk moved from {before_risk_for_after_vendor['overall_risk']:.2f} "
            f"({before_risk_for_after_vendor['risk_level']}) to "
            f"{after_risk_for_after_vendor['overall_risk']:.2f} ({after_risk_for_after_vendor['risk_level']}) "
            f"as a result of the updated requirements."
        )

    # Savings movement for the same vendor
    before_savings_for_after_vendor = next(
        s for s in before_state["savings_list"] if s["name"] == after_name
    )
    after_savings_for_after_vendor = after_state["recommended_savings"]

    if abs(before_savings_for_after_vendor["budget_difference"] - after_savings_for_after_vendor["budget_difference"]) > 0.01:
        parts.append(
            f"Savings for {after_name} shifted from {format_inr(before_savings_for_after_vendor['budget_difference'])} "
            f"to {format_inr(after_savings_for_after_vendor['budget_difference'])} "
            f"({after_savings_for_after_vendor['savings_percentage']:.2f}%) under the new budget."
        )

    return " ".join(parts)


def run_what_if_analysis(vendors, original_weights, new_weights,
                          original_requirements, new_requirements, risk_weights):
    """
    Reusable what-if engine. Computes a BEFORE snapshot and an AFTER snapshot
    using the existing Step 1-3 functions, then compares them.
    """
    before_state = compute_whatif_state(vendors, original_weights, original_requirements, risk_weights)
    after_state = compute_whatif_state(vendors, new_weights, new_requirements, risk_weights)

    weight_changes = diff_weights(original_weights, new_weights)
    requirement_changes = diff_requirements(original_requirements, new_requirements)

    explanation = generate_whatif_explanation(before_state, after_state, weight_changes, requirement_changes)
...
def print_whatif_header():
    print("\n" + "=" * 50)
    print("              WHAT-IF SIMULATION")
    print("=" * 50)


def print_whatif_weights(weights):
    print("Weights:")
    print(
        f"Price {weights['price']}% | Delivery {weights['delivery']}% | "
        f"Quality {weights['quality']}%"
    )
    print(f"Reliability {weights['reliability']}% | Warranty {weights['warranty']}%")


def print_whatif_state(label, state):
    print(f"\n{label}")
    print_whatif_weights(state["weights"])
    print(f"\nRecommended: {state['recommended_name']}")
    print(f"Score: {state['recommended_score']:.2f}")
    print(f"Risk: {state['recommended_risk']['overall_risk']:.2f} ({state['recommended_risk']['risk_level']})")
    print(f"Savings: {format_inr(state['recommended_savings']['budget_difference'])}")


def print_whatif_changes(weight_changes, requirement_changes):
    print("\n" + " " * 16 + "↓")
    print("\nWHAT CHANGED?")

    if not weight_changes and not requirement_changes:
        print("No inputs changed.")
        return

    for crit, old_val, new_val in weight_changes:
        direction = "increased" if new_val > old_val else "decreased"
        print(f"{WEIGHT_LABELS[crit]} {direction} from {old_val}% → {new_val}%")

    for field, old_val, new_val in requirement_changes:
        direction = "increased" if new_val > old_val else "decreased"
        label = REQUIREMENT_LABELS[field]
        if field == "budget":
            print(f"{label} {direction} from {format_inr(old_val)} → {format_inr(new_val)}")
        else:
            print(f"{label} {direction} from {old_val} → {new_val}")

    print("\n" + " " * 16 + "↓")


def run_whatif_scenario(title, vendors, original_weights, new_weights,
                         original_requirements, new_requirements, risk_weights):
    print("\n\n" + "#" * 50)
    print(f"# WHAT-IF TEST: {title}")
    print("#" * 50)
    ...
    print_whatif_state("BEFORE", result["before"])
    print_whatif_changes(result["weight_changes"], result["requirement_changes"])
    print_whatif_state("AFTER", result["after"])
...
if __name__ == "__main__":
    ...
    # TEST 1: Priority change (weights change, requirements stay the same)
    run_whatif_scenario(
        "1. PRIORITY CHANGE (Delivery weight raised)",
        vendors,
        default_weights,             # original weights
        delivery_critical_weights,   # new weights
        requirements,                # original requirements (unchanged)
        requirements,                # requirements stay the same
        risk_weights,
    )

    # TEST 2: Requirement change (requirements change, weights stay the same)
    run_whatif_scenario(
        "2. REQUIREMENT CHANGE (Stricter procurement requirements)",
        vendors,
        default_weights,             # weights stay the same
        default_weights,
        requirements,                 # original requirements
        strict_requirements,          # new, stricter requirements
        risk_weights,
    )
